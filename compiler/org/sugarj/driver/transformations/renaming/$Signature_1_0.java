package org.sugarj.driver.transformations.renaming;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class $Signature_1_0 extends Strategy 
{ 
  public static $Signature_1_0 instance = new $Signature_1_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy j_18)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("Signature_1_0");
    Fail31:
    { 
      IStrategoTerm y_106 = null;
      IStrategoTerm x_106 = null;
      if(term.getTermType() != IStrategoTerm.APPL || renaming._consSignature_1 != ((IStrategoAppl)term).getConstructor())
        break Fail31;
      x_106 = term.getSubterm(0);
      IStrategoList annos13 = term.getAnnotations();
      y_106 = annos13;
      term = j_18.invoke(context, x_106);
      if(term == null)
        break Fail31;
      term = termFactory.annotateTerm(termFactory.makeAppl(renaming._consSignature_1, new IStrategoTerm[]{term}), checkListAnnos(termFactory, y_106));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}