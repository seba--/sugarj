package org.sugarj.driver.transformations.renaming;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class $Package$Or$Type$Name_1_0 extends Strategy 
{ 
  public static $Package$Or$Type$Name_1_0 instance = new $Package$Or$Type$Name_1_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy f_31)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("PackageOrTypeName_1_0");
    Fail214:
    { 
      IStrategoTerm j_149 = null;
      IStrategoTerm i_149 = null;
      if(term.getTermType() != IStrategoTerm.APPL || renaming._consPackageOrTypeName_1 != ((IStrategoAppl)term).getConstructor())
        break Fail214;
      i_149 = term.getSubterm(0);
      IStrategoList annos188 = term.getAnnotations();
      j_149 = annos188;
      term = f_31.invoke(context, i_149);
      if(term == null)
        break Fail214;
      term = termFactory.annotateTerm(termFactory.makeAppl(renaming._consPackageOrTypeName_1, new IStrategoTerm[]{term}), checkListAnnos(termFactory, j_149));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}