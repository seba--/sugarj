package org.sugarj.driver.transformations.extraction;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class real_1_0 extends Strategy 
{ 
  public static real_1_0 instance = new real_1_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy r_29)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("real_1_0");
    Fail276:
    { 
      IStrategoTerm e_149 = null;
      IStrategoTerm d_149 = null;
      if(term.getTermType() != IStrategoTerm.APPL || outt._consreal_1 != ((IStrategoAppl)term).getConstructor())
        break Fail276;
      d_149 = term.getSubterm(0);
      IStrategoList annos227 = term.getAnnotations();
      e_149 = annos227;
      term = r_29.invoke(context, d_149);
      if(term == null)
        break Fail276;
      term = termFactory.annotateTerm(termFactory.makeAppl(outt._consreal_1, new IStrategoTerm[]{term}), checkListAnnos(termFactory, e_149));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}