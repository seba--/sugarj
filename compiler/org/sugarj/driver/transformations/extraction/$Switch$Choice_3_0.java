package org.sugarj.driver.transformations.extraction;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class $Switch$Choice_3_0 extends Strategy 
{ 
  public static $Switch$Choice_3_0 instance = new $Switch$Choice_3_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy m_20, Strategy n_20, Strategy o_20)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("SwitchChoice_3_0");
    Fail92:
    { 
      IStrategoTerm p_115 = null;
      IStrategoTerm l_115 = null;
      IStrategoTerm m_115 = null;
      IStrategoTerm n_115 = null;
      IStrategoTerm q_115 = null;
      IStrategoTerm r_115 = null;
      if(term.getTermType() != IStrategoTerm.APPL || outt._consSwitchChoice_3 != ((IStrategoAppl)term).getConstructor())
        break Fail92;
      l_115 = term.getSubterm(0);
      m_115 = term.getSubterm(1);
      n_115 = term.getSubterm(2);
      IStrategoList annos65 = term.getAnnotations();
      p_115 = annos65;
      term = m_20.invoke(context, l_115);
      if(term == null)
        break Fail92;
      q_115 = term;
      term = n_20.invoke(context, m_115);
      if(term == null)
        break Fail92;
      r_115 = term;
      term = o_20.invoke(context, n_115);
      if(term == null)
        break Fail92;
      term = termFactory.annotateTerm(termFactory.makeAppl(outt._consSwitchChoice_3, new IStrategoTerm[]{q_115, r_115, term}), checkListAnnos(termFactory, p_115));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}