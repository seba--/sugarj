package org.sugarj.driver.transformations.extraction;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class attrs_1_0 extends Strategy 
{ 
  public static attrs_1_0 instance = new attrs_1_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy o_28)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("attrs_1_0");
    Fail245:
    { 
      IStrategoTerm p_144 = null;
      IStrategoTerm m_144 = null;
      if(term.getTermType() != IStrategoTerm.APPL || outt._consattrs_1 != ((IStrategoAppl)term).getConstructor())
        break Fail245;
      m_144 = term.getSubterm(0);
      IStrategoList annos201 = term.getAnnotations();
      p_144 = annos201;
      term = o_28.invoke(context, m_144);
      if(term == null)
        break Fail245;
      term = termFactory.annotateTerm(termFactory.makeAppl(outt._consattrs_1, new IStrategoTerm[]{term}), checkListAnnos(termFactory, p_144));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}