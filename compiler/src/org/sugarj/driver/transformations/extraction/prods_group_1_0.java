package org.sugarj.driver.transformations.extraction;

import org.strategoxt.stratego_lib.*;
import org.strategoxt.lang.*;
import org.spoofax.interpreter.terms.*;
import static org.strategoxt.lang.Term.*;
import org.spoofax.interpreter.library.AbstractPrimitive;
import java.util.ArrayList;
import java.lang.ref.WeakReference;

@SuppressWarnings("all") public class prods_group_1_0 extends Strategy 
{ 
  public static prods_group_1_0 instance = new prods_group_1_0();

  @Override public IStrategoTerm invoke(Context context, IStrategoTerm term, Strategy o_26)
  { 
    ITermFactory termFactory = context.getFactory();
    context.push("prods_group_1_0");
    Fail203:
    { 
      IStrategoTerm p_136 = null;
      IStrategoTerm o_136 = null;
      if(term.getTermType() != IStrategoTerm.APPL || ext_out._consprods_group_1 != ((IStrategoAppl)term).getConstructor())
        break Fail203;
      o_136 = term.getSubterm(0);
      IStrategoList annos167 = term.getAnnotations();
      p_136 = annos167;
      term = o_26.invoke(context, o_136);
      if(term == null)
        break Fail203;
      term = termFactory.annotateTerm(termFactory.makeAppl(ext_out._consprods_group_1, new IStrategoTerm[]{term}), checkListAnnos(termFactory, p_136));
      context.popOnSuccess();
      if(true)
        return term;
    }
    context.popOnFailure();
    return null;
  }
}